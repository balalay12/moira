...
func (connector *DbConnector) removeSubscription(subscription *moira.SubscriptionData) error {
	...
	c.Send("SREM", teamSubscriptionsKey(subscription.Team), subscription.ID) //nolint
...
func addSendSubscriptionRequest(c redis.Conn, subscription moira.SubscriptionData, oldSubscription *moira.SubscriptionData) error {
	...
	if oldSubscription != nil {
		...
		if oldSubscription.Team != subscription.Team {
			c.Send("SREM", teamSubscriptionsKey(oldSubscription.Team), subscription.ID) //nolint
		}
	}
	...
	if subscription.Team != "" {
		c.Send("SADD", teamSubscriptionsKey(subscription.Team), subscription.ID) //nolint
	}
